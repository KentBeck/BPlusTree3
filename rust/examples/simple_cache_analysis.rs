...
fn main() {
    ...
    println!("  BTreeMap: {:.2}ms", btree_sequential.as_secs_f64() * 1000.0);
    println!("  BPlusTreeMap: {:.2}ms", bplus_sequential.as_secs_f64() * 1000.0);
    ...
    println!("  BPlusTreeMap: {:.2}ms", bplus_random.as_secs_f64() * 1000.0);
    ...
    println!("  BTreeMap size: {} bytes", mem::size_of::<BTreeMap<i32, i32>>());
    println!("  BPlusTreeMap size: {} bytes", mem::size_of::<BPlusTreeMap<i32, i32>>());
    ...
    println!("  Small data (1k items): {:.2}ms", small_time.as_secs_f64() * 1000.0);
    println!("  Large data (1M items): {:.2}ms", large_time.as_secs_f64() * 1000.0);
    
    let cache_impact = (large_time.as_secs_f64() / large_data.len() as f64) / 
                      (small_time.as_secs_f64() / small_data.len() as f64);
    println!("  Cache impact factor: {:.2}x slower per item", cache_impact);
    ...
    println!("  Stack size reduction: {}B → {}B", original_stack, optimized_stack);

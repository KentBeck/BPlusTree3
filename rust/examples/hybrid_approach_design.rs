...
fn main() {
    ...
    println!("Compact arena (safe): {:?} ({:.2}x vs BTreeMap)", 
             compact_safe_time, 
             compact_safe_time.as_nanos() as f64 / btree_time.as_nanos() as f64);
    println!("Compact arena (unsafe): {:?} ({:.2}x vs BTreeMap)", 
             compact_unsafe_time, 
    ...
    println!("Hybrid Box iteration: {:?} ({:.2}x vs BTreeMap)", 
             hybrid_time, 
    ...
    println!("Memory pool iteration: {:?} ({:.2}x vs BTreeMap)", 
             pool_time, 
    ...
    println!("Compact arena size: {} bytes", std::mem::size_of_val(&compact_arena));
    println!("Hybrid tree size: {} bytes", std::mem::size_of_val(&hybrid_tree));
    ...
    println!("Random access: {:?} ({:.2}x slower)", 
             random_time, 

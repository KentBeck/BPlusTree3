...
fn main() {
    ...
    for &size in &[1, 2, 5, 10, 20] {
        ...
        println!("Size {}: BTree {:.2}µs | BPlus {:.2}µs ({:.1}x) | Fast {:.2}µs ({:.1}x)", 
                 size,
    ...
    for &size in &[100, 1000, 5000] {
        ...
        let btree_seq_delete = benchmark_operation("BTreeMap Sequential Delete", 100, || {
            ...
            for i in 0..size { tree.insert(i, i); }
            for i in 0..size { tree.remove(&i); }
            black_box(tree);
        });
        ...
        let btree_rand_delete = benchmark_operation("BTreeMap Random Delete", 100, || {
            let mut tree = BTreeMap::new();
            for i in 0..size { tree.insert(i, i); }
            for &key in &delete_keys { tree.remove(&key); }
            black_box(tree);
        });
        ...
        println!("  Sequential: BTree {:.2}ms | BPlus {:.2}ms ({:.1}x slower)", 
        ...
        println!("  Random:     BTree {:.2}ms | BPlus {:.2}ms ({:.1}x slower)", 
    ...
    for &range_size in &[1, 10, 100, 1000, 5000] {
        ...
        println!("Range {}: BTree {:.2}µs | BPlus {:.2}µs ({:.1}x slower)", 
                 range_size,
    ...
    println!("Many small trees: BTree {:.2}ms | BPlus {:.2}ms ({:.1}x slower)", 
    ...
    println!("Per-tree overhead: BTree {}B | BPlus {}B ({:.1}x more)", 
             btree_mem, bplus_mem, bplus_mem as f64 / btree_mem as f64);
    ...
    let alternating_keys: Vec<i32> = (0..size).map(|i| if i % 2 == 0 { i } else { size - i }).collect();
    ...
    println!("Alternating access: BTree {:.2}ms | BPlus {:.2}ms ({:.1}x slower)", 
    ...
    println!("Mixed workload: BTree {:.2}ms | BPlus {:.2}ms ({:.1}x slower)", 

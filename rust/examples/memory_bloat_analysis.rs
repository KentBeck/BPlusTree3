...
fn main() {
    ...
    println!(
        "BPlusTreeMap<i32, i32> total: {} bytes",
        mem::size_of::<BPlusTreeMap<i32, i32>>()
    );
    println!(
        "BTreeMap<i32, i32> total: {} bytes",
        mem::size_of::<BTreeMap<i32, i32>>()
    );
    println!(
        "Difference: {} bytes ({:.1}x larger)",
        mem::size_of::<BPlusTreeMap<i32, i32>>() - mem::size_of::<BTreeMap<i32, i32>>(),
        mem::size_of::<BPlusTreeMap<i32, i32>>() as f64
            / mem::size_of::<BTreeMap<i32, i32>>() as f64
    );
    ...
    println!(
        "Empty BPlusTreeMap: {} bytes",
        mem::size_of_val(&empty_tree)
    );
    ...
    println!(
        "Single element BPlusTreeMap: {} bytes",
        mem::size_of_val(&single_tree)
    );
    ...
    println!(
        "Single element BTreeMap: {} bytes",
        mem::size_of_val(&single_btree)
    );
    ...
    println!(
        ...
        mem::size_of::<usize>() + 2 * mem::size_of::<Vec<i32>>() + mem::size_of::<u32>()
    );
    ...
    println!("  keys (Vec<i32>): {} bytes", mem::size_of::<Vec<i32>>());
    println!(
        "  children (Vec<NodeRef>): {} bytes",
        mem::size_of::<Vec<u64>>()
    ); // Approximate
    println!(
        "  Total overhead: ~{} bytes",
        mem::size_of::<usize>() + mem::size_of::<Vec<i32>>() + mem::size_of::<Vec<u64>>()
    );
    ...
    println!(
        "1. PhantomData in NodeRef: {} bytes wasted per reference",
        mem::size_of::<std::marker::PhantomData<(i32, i32)>>()
    );
    println!(
        "2. Separate capacity in each node: {} bytes per node",
        mem::size_of::<usize>()
    );
    println!(
        "3. Vec overhead: {} bytes per Vec",
        mem::size_of::<Vec<i32>>()
    );
    ...
    for &size in &[1, 5, 10, 20, 50] {
        ...
        println!(
            "Size {}: Current {:.2}x overhead, Optimized {:.2}x ({:.1}% improvement)",

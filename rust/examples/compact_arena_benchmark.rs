...
fn main() {
    ...
    for &capacity in &[16, 32, 64, 128] {
        ...
        println!("Capacity {}: Regular {:?} ({:.2}x vs BTreeMap), Fast {:?} ({:.2}x vs BTreeMap)", 
                 capacity,
                 regular_time, 
    ...
    println!("BPlusTreeMap insertion: {:?} ({:.2}x vs BTreeMap)", 
             bplus_insert_time,
    ...
    println!("BPlusTreeMap lookups: {:?} ({:.2}x vs BTreeMap)", 
             bplus_lookup_time,
    ...
    println!("BPlusTreeMap range queries: {:?} ({:.2}x vs BTreeMap)", 
             bplus_range_time,
    ...
    println!("BPlusTreeMap regular per item: {:.2} ns ({:.2}x overhead)", 
             regular_per_item, regular_per_item / btree_per_item);
    println!("BPlusTreeMap fast per item: {:.2} ns ({:.2}x overhead)", 
             fast_per_item, fast_per_item / btree_per_item);
    ...
    println!("Sequential Vec access: {:?} ({:.2} ns/item)", 
             sequential_time, 
             sequential_time.as_nanos() as f64 / (iterations * size) as f64);
    println!("Random Vec access: {:?} ({:.2} ns/item, {:.2}x slower)", 
             random_time,
    ...
    println!("- Regular iteration: {:.1}% {} than BTreeMap", 
             (regular_per_item / btree_per_item - 1.0).abs() * 100.0,
             if regular_per_item < btree_per_item { "faster" } else { "slower" });
    println!("- Fast iteration: {:.1}% {} than BTreeMap", 
             (fast_per_item / btree_per_item - 1.0).abs() * 100.0,
             if fast_per_item < btree_per_item { "faster" } else { "slower" });
    println!("- Fast vs Regular: {:.1}% improvement", 
